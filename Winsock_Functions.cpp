...
void create_test(int log_file_num) {
	int index = 0;
	int line_counter = 1;
	double x, y, z;
	float ADC_ICD_data[30000], ADC_ICD_Alg_data[30000];
	int shock = 0;
	int voltage_counter = 0;
	int shock_ALG = 0;
	char flagfile = 0;
	//Test file creating
	std::ofstream testlog("test_file.txt");
	//Name of EGM file from log file
	std::string log_file_name;
	std::string infile;
	std::ifstream stream_log;
	//write the file header
	testlog << "EGM File Name" << "\t" << "ICD Device" << "\t" << "ICD Algorithm" << std::endl << std::endl;
	//loop for the number of files
	for (int file_num = (log_file_num); file_num>0; file_num--) {

		//loop for each ICD case, device and algorithm
		for (int ICD_ind = 0; ICD_ind < 2; ICD_ind++) {

			index = 0;


			//open the correct log file to read
			if (ICD_ind == 0) stream_log.open(("log_file_ICD" + std::to_string(file_num) + ".txt").c_str());
			else stream_log.open(("log_file_Open_ICD" + std::to_string(file_num) + ".txt").c_str());
			//getting lines from the log file
			for (infile; getline(stream_log, infile, 'd');) {

				std::istringstream file_in(infile);
				//put the first line into a string for the EGM file name
				if (line_counter == 1) {
					if (flagfile == 0) {
						file_in >> log_file_name;
						flagfile = 1;
					}

				}
				//If not first line fill the ADC buffers, wait till the fourth line
				else if (line_counter >= 4) {
					if (ICD_ind == 0) {
						file_in >> x >> y >> z >> ADC_ICD_data[index];
						

					}
					else file_in >> x >> y >> z >> ADC_ICD_Alg_data[index];
					index++;
			...
			stream_log.close();
			index = 0;
			//checking the shock data
			for (index; index < 30000; index++) {
				if (ICD_ind == 0) {
					if (ADC_ICD_data[index] >= 3.2) voltage_counter++;
					else if (ADC_ICD_data[index] < 3.2 &&  voltage_counter >= 4) {
						shock = 1;
						voltage_counter = 0;
					}
					...
					if (ADC_ICD_Alg_data[index] >= 3.2) voltage_counter++;
					else if (ADC_ICD_Alg_data[index] < 3.2 &&  voltage_counter >= 4) {
						shock_ALG = 1;
						voltage_counter = 0;
					}
					else voltage_counter = 0;
		...
		//write the results 0 means no shock, 1 means shock
		testlog << log_file_name << "   \t" << shock << "\t\t" << shock_ALG << std::endl;
		//reset the shock variables
		//reset shock flags
		shock = 0;
		shock_ALG = 0;
